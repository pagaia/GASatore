...
if($_POST['edit']){
	...
	$founds = "<pre>";
        ...
        $founds .= "User Booking Id: ".$_POST['ubooking_id']."\n";
...
// first create a new userBooking
	$itemsList = array();
	$userBooking = new userBooking($db, $log, $_POST['ubooking_id']);
	$userBooking->update($_POST['user_id'], $_POST['booking_date_id'], $_POST['pickup_date_id'],  $_POST['total_cost'], $_POST['total_paied'], $_POST['return2user'], $_POST['total_cache'], $_POST['debito_credito']);
	...
	$userBooking->updateItems($itemsList);
                  
	echo "<h2> Prenotazione modificata con successo! Attendi l'aggiornamento</h2>";
#	echo "<script > 
...
//	}
	...
	//load UserBooking info
	$uBooking = new userBooking($db, $log, $_GET['id']);
	...
	echo "<input type='hidden' class='tnumber' name='ubooking_id' id='ubooking_id' value='$uBooking->id'/>\n";
	echo "<fieldset>
	...
	echo "<div class='row'><label for='user_id'>User:</label><span class='formw'>\n";
        $userList = User::listUser($db, $log);
                ...
                for ($i = 0; $i < $count; $i++) {
			$u = $userList[$i];
                        echo '<option value="'.$u->id.'" '.(($uBooking->user->id == $u->id)?'selected':'').'>'.$u->surname.' '.$u->name.'</option>\n';
                }
                echo '</select>';
        ...
        $bookingCal = calendar::listCalendar($db, $log, 1);
                ...
                for ($i = 0; $i < $count; $i++) {
                        $cal = $bookingCal[$i];
                        echo '<option value="'.$cal->id.'" '.(($uBooking->booking_date_id->id == $cal->id)?'selected':'').'>'.strftime("%a %d %B %G", strtotime("$cal->day")).'</option>\n';
                }
                echo '</select>';
	...
	echo "<div class='row'><label for='pickup_date_id'>Pickup day:</label><span class='formw'>\n";
        $pickupCal = calendar::listCalendar($db, $log, 1);
                ...
                for ($i = 0; $i < $count; $i++) {
                        $cal = $pickupCal[$i];
                        echo '<option value="'.$cal->id.'" '.(($uBooking->pickup_date_id->id == $cal->id)?'selected':'').' >'.strftime("%a %d %B %G", strtotime("$cal->day")).'</option>\n';
                }
                echo '</select>';
...
//	$donationPaied = new donationPaied($db, $log, null, 1 );
	...
	foreach($categories as $cat){
		...
		if(count($productsList)>0){
			...
			foreach($productsList as $product){
				...
				<input type='text' class='tnumber' name='p_$product->id' id='p_$product->id' value='". $mybook->quantity ."'/>
				<input type='text' class='tnumber' name='price_$product->id'  id='price_$product->id' value='".(( floatval($product->price) > 0)? $product->price : "0.00")."'  readOnly='true'/>
				<input type='text' class='tnumber red' name='tprice_$product->id' id='tprice_$product->id'  value='". (($mybook->tot_price > 0)?$mybook->tot_price:"0.00") ."' readOnly='true'/>
				<span id='p_{$product->id}checker'>checker</span>
				</div>\n";
	...
	echo "<br><div class='row'>
                ...
                <input type='text' class='tnumber red' name='total_cost' id='total_cost'  value='". ($uBooking->owed > 0 ? $uBooking->owed : "0.00") ."' readonly='true'/>
                </div>\n";
	echo "<div class='row'>
                <label for='total_paied' >Totale pagato </label>
                <input type='text' class='tnumber ' name='total_paied' id='total_paied'  value='". ($uBooking->paied > 0 ?$uBooking->paied : "0.00" )."' />
                </div>\n";
	echo "<div class='row'>
	...
	echo "<div class='row'>
                ...
                <input type='text' class='tnumber' name='return2user' id='return2user'  value='".( $uBooking->changedback > 0 ? $uBooking->changedback : "0.00"). "' />
                </div>\n";
	echo "<div class='row'>
                <label for='total_cache' >Totale cassa</label>
                <input type='text' class='tnumber' name='total_cache' id='total_cache'  value='".( $uBooking->total_cache > 0 ? $uBooking->total_cache : "0.00"). "' readonly='true'/>
                </div>\n";
	echo "<div class='row'>
                <label for='debito_credito' >Debito/Credito gaabista</label>
                <input type='text' class='tnumber' name='debito_credito' id='debito_credito'  value='$uBooking->debit_credit' readonly='true'/>
                </div>\n";
...
*/
	...
	echo "<input type='reset' name='reset' value='Resetta campi' />\n";
        echo "        <input type='submit' name='edit' value='Prenota' />\n";
